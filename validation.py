...
def _first_error(exc) -> str:
    errors = exc.errors()
    return errors[0]["msg"].removeprefix("Value error, ") if errors else "Validation error"

def _errors_list(exc) -> list:
...
class UserRegistrationSchema(NameEmailMixin):
...
class NomineeSchema(_NomineeBase):
    aadhaar_number: str
...
class ApiNomineeSchema(_NomineeBase):
...
class _CustomerCreateBase(NameEmailMixin, DobMixin):
    full_name:    str = Field(min_length=2, max_length=100)
    gender:       GENDERS = ""
    email:        EmailStr
    phone:        str
...
class CustomerCreateSchema(_CustomerCreateBase):
    flat_no:           Optional[str] = None
    block_number:      Optional[str] = None
...
class ApiCustomerCreateSchema(_CustomerCreateBase):
    address:        Optional[dict] = None
    parent_details: Optional[dict] = None
    nominees:       list[ApiNomineeSchema] = Field(default_factory=list)


# ── Customer edit schemas ──────────────────────────────────────────────────

class CustomerEditSchema(NameEmailMixin):
    full_name:         str = Field(min_length=2, max_length=100)
    gender:            GENDERS = ""
    email:             EmailStr
    phone:             str
...
def validate_user_registration(form):
    try:
        data = UserRegistrationSchema(full_name=form.get("full_name", ""),
...
def validate_customer_form(form):
    ...
    if add_nominees:
        for i in range(1, 4):
            ...
            if not any((name, relation, aadhaar)):
                continue
            nominees.append({
                "nominee_name":   name, "relation": relation, "aadhaar_number": aadhaar,
    ...
    try:
        data = CustomerCreateSchema(
            full_name=form.get("full_name", ""), dob=form.get("dob", ""),
            gender=form.get("gender", "") or "", email=form.get("email", ""),
            phone=form.get("phone", ""), account_type=form.get("account_type", ""),
            aadhaar=form.get("aadhaar", ""), pan=form.get("pan", ""),
            flat_no=form.get("flat_no") or None, block_number=form.get("block_number") or None,
            society_name=form.get("society_name") or None, street=form.get("street") or None,
            city=form.get("city") or None, state=form.get("state") or None,
        ...
        return True, {
            "full_name": data.full_name, "dob": data.dob_date, "age": data.age,
            "gender": data.gender or None, "email": data.email, "phone": data.phone,
            "account_type": data.account_type, "aadhaar": data.aadhaar, "pan": data.pan,
            "flat_no": data.flat_no, "block_number": data.block_number,
            "society_name": data.society_name, "street": data.street,
            "city": data.city, "state": data.state, "pincode": data.pincode,
            "father_name": data.father_name, "father_occupation": data.father_occupation,
            "mother_name": data.mother_name, "mother_type": data.mother_type or None,
            "nominees": [{"nominee_name": n.nominee_name, "relation": n.relation,
                          "aadhaar_number": n.aadhaar_number, "phone_number": n.phone_number,
                          "email": str(n.email) if n.email else None,
                          "flat_no": n.flat_no, "block_number": n.block_number,
                          "city": n.city, "state": n.state, "pincode": n.pincode}
...
def validate_edit_customer_form(form):
    try:
        data = CustomerEditSchema(
            full_name=form.get("full_name", ""), gender=form.get("gender", "") or "",
        ...
        return True, {
            "full_name": data.full_name, "gender": data.gender or None,
            "email": data.email, "phone": data.phone, "account_type": data.account_type,
            "flat_no": data.flat_no, "block_number": data.block_number,
            "society_name": data.society_name, "street": data.street,
            "city": data.city, "state": data.state, "pincode": data.pincode,
            "father_name": data.father_name, "father_occupation": data.father_occupation,
            "mother_name": data.mother_name, "mother_type": data.mother_type or None,
            "document_verified": data.document_verified, "risk_category": data.risk_category,
        }
    except Exception as exc:
        return False, _first_error(exc)
...
def validate_api_customer_create(data: dict):
    try:
        ...
        return True, {"full_name": m.full_name, "dob": m.dob_date, "age": m.age,
                      "gender": m.gender or None, "email": m.email, "phone": m.phone,
                      "account_type": m.account_type, "aadhaar": m.aadhaar, "pan": m.pan,
                      "address": m.address, "parent_details": m.parent_details,
...
def validate_api_customer_update(data: dict):
    try:
        ...
        return True, {k: v for k, v in {
            "full_name": m.full_name, "gender": m.gender or None,
            "email": m.email, "phone": m.phone, "account_type": m.account_type,
            "address": m.address, "parent_details": m.parent_details,
